# A Client GUI for Playing Boggle
## Objective
...
After installing Visual Studio, run the following command to clone the repo locally on your machine. The project should be cloned into a folder located in your current working directory called "BoggleClient". However, if you don't like the command line, you can also follow Visual Studio's built-in repo cloner and clone it that way.

`git clone https://github.com/Hawkbawk/BoggleServer`

Then, simply open up Visual Studio, navigate to wherever you cloned the project, open it up and voila! You should be able to load up the solution and see all of the projects and files. To actually run the client on, simply hit the run button and the GUI will pop up. The default server (http://ice.eng.utah.edu) was still running as of September 15, 2019, but if it goes down, simply clone the BoggleServer repo and run it in a separate instance of Visual Studio. After that, simply type in your desired username, register yourself, type in your desired time limit, then hit play. Two clients need to connect at once in order to play a game, so to see the full functionality, you'll need to have a server and two clients running, which could mean having multiple instances of Visual Studio open at once.

## About
This BoggleClient project was written by myself, Ryan Hawkins, and a fellow student at the University of Utah, Curtis Lin. The original application was written during the spring of 2019, in the CS 3500 Software Practice class at the University of Utah, taught by Joe Zachary. Some of the code in this repo was written by him, and accordingly has a header crediting him. This application was built over the course of 3 to 4 weeks, with each week building the codebase. Eventually, we had created a client that followed the API outlined at http://ice.eng.utah.edu/api.html. This client can communicate with any server that follows the outlined API.